...
const event_api = "http://localhost:8003";
const main_api = "http://localhost:8002";
const qr_api = "http://localhost:8001";
...
describe(`Running tests for ${auth_api} server for Astrix Authentications...`, () => {
  describe("Checking server running health status", () => {
    test("test 1: checking if server running or not", async () => {
      try {
        const res = authMethods.serverHealth();
      } catch (error) {}
    });
  });
  ...
  describe("Checking for user registration: /register", () => {
    test("test 1: creating a user", async () => {
      await authMethods.registerUser(user1); // new user registration
    });
